...
def creat_word_embedding():
    if os.path.exists(args.project_dir + 'Data/word_emb_google.pkl'):
        return
    if not os.path.exists(args.project_dir + 'GoogleNews-vectors-negative300.bin.gz'):
    ...
    model = word2vec.KeyedVectors.load_word2vec_format(args.project_dir + 'GoogleNews-vectors-negative300.bin.gz',
                                                       binary=True)
